...
const APP_META = {
  name: "RON SCRIPTS",
  version: "3.0.0-ACCESS-GATE",
  build: "RON-ULTIMATE-V3-2026.08.24.1",
  channel: "ULTIMATE",
  release: "VIP • TESTER • CUSTOM LAB • UI",
  updated: "2026-08-23"
};
...
function setGateStep(step) {
  ...
  $$(".gate-step").forEach(card => {
    const active = Number(card.dataset.stepCard) === step;
    card.classList.toggle("active", active);
    card.classList.toggle("done", Number(card.dataset.stepCard) < step);
  });
  [1,2,3].forEach(n => {
    const el = $(`#gate-status-${n}`);
    if (!el) return;
    if (n < step || (n === 2 && gateState.captchaPassed)) el.textContent = "DONE";
    else if (n === step) el.textContent = "ACTIVE";
    else el.textContent = "LOCKED";
  });
  if (step === 2) mountTurnstile();
}

function mountTurnstile() {
  const holder = $("#turnstile-widget");
  const note = $("#gate-captcha-note");
  if (!holder) return;
  if (typeof window.turnstile === "undefined") {
    if (note) {
      note.hidden = false;
      note.textContent = "CAPTCHA service is still loading. Wait a moment and try again.";
    }
    setTimeout(mountTurnstile, 900);
    return;
  }
  if (gateState.widgetId !== null) return;
  if (!TURNSTILE_SITE_KEY || TURNSTILE_SITE_KEY.includes("PASTE_YOUR")) {
    if (note) {
      note.hidden = false;
      note.innerHTML = "Add your real Cloudflare Turnstile <strong>site key</strong> in <code>script.js</code> before publishing this gate.";
    }
    return;
  }
  holder.innerHTML = "";
  gateState.widgetId = window.turnstile.render(holder, {
    sitekey: TURNSTILE_SITE_KEY,
    size: "flexible",
    theme: "auto",
    callback: token => {
      gateState.captchaPassed = true;
      gateState.captchaToken = token;
      const status = $("#gate-status-2");
      if (status) status.textContent = "DONE";
      const next = $("#gate-next-2");
      if (next) next.disabled = false;
      const note = $("#gate-captcha-note");
      if (note) { note.hidden = false; note.textContent = "CAPTCHA passed. Continue to the main page."; }
    },
    "expired-callback": () => {
      gateState.captchaPassed = false;
      gateState.captchaToken = "";
      const next = $("#gate-next-2");
    ...
    "error-callback": () => {
      ...
      gateState.captchaToken = "";
      const note = $("#gate-captcha-note");
      if (note) { note.hidden = false; note.textContent = "CAPTCHA could not be loaded. Check the Turnstile site key and network connection."; }
    }
  });
}
...
function bootGate() {
  ...
  const follow = $("#quest-follow");
  const next1 = $("#gate-next-1");
  const next2 = $("#gate-next-2");
  const back2 = $("#gate-back-2");
  const launch = $("#gate-launch-btn");
  const reset = $("#gate-reset");
  if (!sub || !follow || !next1) return;

  const syncStep1 = () => {
    next1.disabled = !(sub.checked && follow.checked);
    const status = $("#gate-status-1");
    if (status) status.textContent = next1.disabled ? "WAITING" : "READY";
  };

  sub.addEventListener("change", syncStep1);
  follow.addEventListener("change", syncStep1);
  next1.addEventListener("click", () => setGateStep(2));
  back2?.addEventListener("click", () => setGateStep(1));
  next2?.addEventListener("click", () => {
    if (!gateState.captchaPassed) return;
    setGateStep(3);
    const status = $("#gate-status-3");
...
function bootMainUI() {
  try {
    ...
    const build = $("#quick-build");
    if (build) build.textContent = APP_META.version.replace("-ACCESS-GATE", "");
    fillFilters();
    hydrateControls();
    updateVipUI();
    ...
    updateDebugPanel("ui ready");
  } catch (error) {
    state.lastError = error?.stack || String(error);
    console.error("RON UI initialization failed:", error);
    updateDebugPanel("init error");
    const result = $("#result-summary");
    if (result) result.textContent = "Library failed to start. Open Tester for diagnostics.";
  }
}
