...
// =========================================================
const GATE_STORAGE_KEY = "ron_gate_complete_v3_2";
const QUEST_MIN_SECONDS = 8;
const QUEST_COUNT = 3;

const QUEST_POOL = [
  { id: "yt_sub", platform: "YouTube", title: "Subscribe @NCRONNMLBB", subtitle: "Open the creator channel", icon: "fa-brands fa-youtube", url: "https://www.youtube.com/@NCRONNMLBB" },
  { id: "tt_follow", platform: "TikTok", title: "Follow @ranzee.ron", subtitle: "Open the creator profile", icon: "fa-brands fa-tiktok", url: "https://www.tiktok.com/@ranzee.ron" },
  { id: "yt_profile", platform: "YouTube", title: "Visit @NCRONNMLBB", subtitle: "Spend a few seconds on the channel", icon: "fa-solid fa-user-check", url: "https://www.youtube.com/@NCRONNMLBB" },
  { id: "tt_profile", platform: "TikTok", title: "Visit @ranzee.ron", subtitle: "Spend a few seconds on the profile", icon: "fa-solid fa-user-check", url: "https://www.tiktok.com/@ranzee.ron" },
  { id: "yt_video", platform: "YouTube", title: "Open a creator page", subtitle: "Review the latest upload area", icon: "fa-solid fa-play", url: "https://www.youtube.com/@NCRONNMLBB/videos" },
  { id: "tt_videos", platform: "TikTok", title: "Open creator videos", subtitle: "Review the creator's posts area", icon: "fa-solid fa-video", url: "https://www.tiktok.com/@ranzee.ron" }
];
...
function newChallenge() {
  const modes = ["math", "sequence", "word"];
  const mode = modes[Math.floor(Math.random() * modes.length)];
  questState.round += 1;

  if (mode === "math") {
    const a = 3 + Math.floor(Math.random() * 17);
    const b = 2 + Math.floor(Math.random() * 13);
    const op = Math.random() > .5 ? "+" : "×";
    const answer = op === "+" ? a + b : a * b;
    return { question: `What is ${a} ${op} ${b}?`, answer: String(answer) };
  }
  ...
  if (mode === "sequence") {
    ...
    return { question: `Continue the sequence: ${values.join(", ")}, ?`, answer: String(start + step * 4) };
  }

  const words = ["NOVA", "RON", "ARGUS", "SCRIPT", "VAULT", "VIP"];
  const word = words[Math.floor(Math.random() * words.length)];
  const shuffledWord = shuffle(word.split(""));
  return { question: `Unscramble: ${shuffledWord.join(" ")}`, answer: word };
}

function renderQuest() {
  const list = $("#quest-list");
  if (!list) return;
  list.innerHTML = questState.tasks.map((task, index) => `
    <article class="quest-item" data-quest-id="${task.id}">
      ...
      <div class="quest-item-main">
        ...
        <small class="quest-item-state" data-state="${task.id}">Not started</small>
      </div>
      <button class="quest-open" data-quest-open="${task.id}" type="button"><i class="fa-solid fa-arrow-up-right-from-square"></i> Open</button>
    </article>
  `).join("");
...
function questProgress() {
  ...
  if (text) text.textContent = `${done} / ${QUEST_COUNT} complete`;
  if (intro) intro.textContent = done === QUEST_COUNT ? "All mission tasks passed. Solve the live challenge to unlock the library." : `Complete all ${QUEST_COUNT} randomly selected tasks. Each task needs a real return to this page after a short dwell time.`;
  if (next) next.disabled = !(done === QUEST_COUNT && questState.challengePassed);

  const status = $("#gate-status-1");
  if (status) status.textContent = done === QUEST_COUNT && questState.challengePassed ? "READY" : done ? "ACTIVE" : "WAITING";
}

function markTaskLive(id) {
  const task = questState.tasks.find(t => t.id === id);
  if (!task || questState.completed.has(id)) return;

  questState.currentTask = id;
  questState.openedAt.set(id, Date.now());

  const stateEl = $(`[data-state="${id}"]`);
  if (stateEl) stateEl.textContent = `LIVE CHECK • open the task, then return`;

  const timerText = $("#quest-timer-text");
  if (timerText) timerText.textContent = "Live check: waiting for return";
  ...
  const finishWhenReady = () => {
    if (completed || !leftPage) return;
    const elapsed = Math.floor((Date.now() - questState.openedAt.get(id)) / 1000);
    const remaining = Math.max(0, QUEST_MIN_SECONDS - elapsed);
    if (timerText) timerText.textContent = remaining > 0 ? `Live check: return timer ${remaining}s` : "Live check: verifying return";
    if (remaining <= 0) {
      completed = true;
      cleanup();
      completeTask(id);
  ...
  const onBlur = () => {
    leftPage = true;
    if (timerText) timerText.textContent = `Live check: return in ${QUEST_MIN_SECONDS}s`;
  };
  ...
  const onFocus = () => finishWhenReady();

  window.addEventListener("blur", onBlur);
  window.addEventListener("focus", onFocus);
  document.addEventListener("visibilitychange", onVisibility);

  timer = setInterval(finishWhenReady, 500);
}
...
function completeTask(id) {
  ...
  const stateEl = $(`[data-state="${id}"]`);
  if (stateEl) stateEl.textContent = "Verified in real time";
...
function renderChallenge() {
  questState.challenge = newChallenge();
  questState.challengePassed = false;
  ...
  const submit = $("#challenge-submit");
  if (round) round.textContent = `ROUND ${String(questState.round).padStart(2, "0")}`;
  if (question) question.textContent = questState.challenge.question;
  if (answer) { answer.value = ""; answer.disabled = questState.completed.size !== QUEST_COUNT; }
  if (submit) submit.disabled = questState.completed.size !== QUEST_COUNT;
  if (feedback) feedback.textContent = questState.completed.size === QUEST_COUNT ? "Mission complete. Solve the random challenge." : "The challenge unlocks after all mission tasks are completed.";
}

function verifyChallenge() {
  const input = $("#challenge-answer");
  const feedback = $("#challenge-feedback");
  if (!input || !questState.challenge || questState.completed.size !== QUEST_COUNT) return;
  const given = input.value.trim().toUpperCase();
  const expected = String(questState.challenge.answer).toUpperCase();

  if (given === expected) {
    questState.challengePassed = true;
    if (feedback) {
      feedback.textContent = "Challenge verified. Your quest is complete.";
      feedback.style.color = "#44ff93";
    }
    questProgress();
  } else {
    questState.challengePassed = false;
    if (feedback) {
      feedback.textContent = "Incorrect. A new challenge has been generated.";
...
function setGateStep(step) {
  ...
  [1, 2].forEach(n => {
    const el = $(`#gate-status-${n}`);
    if (!el) return;
    if (n < step) el.textContent = "DONE";
    else if (n === step) el.textContent = "ACTIVE";
    else el.textContent = "LOCKED";
  });
}

function unlockGate() {
  if (!(questState.completed.size === QUEST_COUNT && questState.challengePassed)) return;
...
function bootGate() {
  ...
  const next = $("#gate-next-1");
  const launch = $("#gate-launch-btn");
  const reset = $("#gate-reset");
  const challengeSubmit = $("#challenge-submit");

  questState.tasks = pickQuestTasks();
  questState.completed.clear();
  questState.openedAt.clear();
  questState.round = 0;
  questState.challengePassed = false;
  renderQuest();
  renderChallenge();
  ...
  $("#challenge-answer")?.addEventListener("keydown", e => {
  ...
  next?.addEventListener("click", () => {
    if (!(questState.completed.size === QUEST_COUNT && questState.challengePassed)) return;
    setGateStep(2);
    const status = $("#gate-status-2");
    if (status) status.textContent = "READY";
  });

  launch?.addEventListener("click", unlockGate);
  ...
  window.addEventListener("visibilitychange", () => {
    if (!document.hidden && questState.currentTask) {
      const id = questState.currentTask;
      const started = questState.openedAt.get(id) || 0;
      if (Date.now() - started >= QUEST_MIN_SECONDS * 1000) completeTask(id);
    }
  });
}
